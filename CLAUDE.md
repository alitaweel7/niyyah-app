...
- `Runner/Gating/GateMethodChannel.swift` — method channel **`com.ayaunlock/gate`**: hasPermissions, requestPermissions, showAppPicker, startGating, grantTemporaryUnlock, revokeUnlock, getUnlockState, checkGateRequested, getGateNavigationInfo, syncPrefsToAppGroup, syncShieldAyahs. (Event channel `com.ayaunlock/gate_events` exists but is unused.)
- `Runner/Gating/FamilyControlsBridge.swift` — ManagedSettingsStore shields, selection persistence, and the "five layers, one source of truth" re-lock: `unlock_until` epoch in the App Group + idempotent `enforceLockState()` (usage-threshold DeviceActivityEvent, padded midnight-safe backstop schedule, foreground/clock-change hooks, expiry notification, daily `niyyah_watchdog`). Enforcement never removes shields; granting an unlock is the only removal path.
- Extensions: `ShieldConfigurationExtension` (renders the shield: rotating ayah + "Read to Unlock"), `ShieldActionExtension` (button tap → time-sensitive notification → app opens the gate; ShieldAction *cannot* launch apps directly — the notification indirection is an iOS platform limit), `DeviceActivityMonitorExtension` (threshold/interval/watchdog callbacks → same enforcement, guarded by `unlock_until`).
- **App Group `group.com.ayaunlock.shared`** is the only Dart↔extension bridge. Keys: `gate_selection` (full FamilyActivitySelection JSON), `shielded_app_tokens` (legacy, kept in sync one release), `unlock_until`, `last_relock_at`/`last_relock_reason`, `gate_duration_seconds`, `unlock_duration_seconds`, `preferred_content_type`, `shield_ayahs`, `gate_requested_at`, `last_blocked_app_name`.
